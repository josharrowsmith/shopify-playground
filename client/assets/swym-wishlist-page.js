...
var productCardMarkup = `
 ...
 {{#products}}
    ...
    <a href="" data-product-id="{{empi}}" data-url="{{du}}" data-variant-id="{{epi}}" class="remove">
    {{^isInCart}}remove{{/isInCart}}
    </a>
  {{/products}}
 </ul>
`;
...
function removeProduct(e){
  ...
  var variantId = e.target.getAttribute("data-variant-id");
  var du = e.target.getAttribute("data-url");
  console.log(du, productId, variantId, )
  e.target.innerHTML = "removing..";
  window._swat.removeFromWishList(
   {empi: productId, du: du, pr: 1.00},
   variantId,
   function() {
     window.location.reload();
...
function attachClickListeners(){
  ...
  for (var i = 0; i < removeButtons.length; i++) {
    ...
    removeButtons[i].addEventListener('click', removeProduct, false);
  }
}